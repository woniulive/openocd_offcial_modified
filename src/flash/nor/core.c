	...
	uint32_t *written, int erase, bool unlock)
{
	...
	while (section < image->num_sections) {
    	...
    	{
        	LOG_INFO("Programming FLASH section %d/%d (%d bytes) at 0x%08x...", section + 1, image->num_sections, sections[section]->size, (unsigned int)sections[section]->base_address);
    	}
