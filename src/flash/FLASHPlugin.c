...
int save_region(struct target *target, struct memory_backup *region, uint32_t sh_addr, uint32_t sh_size, int sectionNumber)
{
...
static int plugin_erase(struct flash_bank *bank, int first, int last)
{
    ...
    if (retval == ERROR_OK)
    {
        ...
        {
            int32_t result;
            retval = call_plugin_func(target, loaded_plugin.timeouts.erase, plugin_info->FLASHPlugin_EraseSectors, loaded_plugin.sp, (uint32_t *)&result, 2, first, last - first + 1);
            if (retval != ERROR_OK)
                break;
            if (result < 0)
            {
...
static int plugin_protect(struct flash_bank *bank, int set, int first, int last)
{
    ...
    if (retval == ERROR_OK)
    {
        ...
        {
            int32_t result;
            retval = call_plugin_func(target, loaded_plugin.timeouts.protect, plugin_info->FLASHPlugin_ProtectSectors, loaded_plugin.sp, (uint32_t *)&result, 3, set, first, last - first + 1);
            if (retval != ERROR_OK)
                break;
            if (result < 0)
            {
    ...
    if (retval == ERROR_OK)
    {
        ...
        if (retval == ERROR_OK)
        {
            for (int sector =  0; sector < bank->num_sectors;)
            {
                ...
                int32_t result;
                retval = call_plugin_func(target, loaded_plugin.timeouts.protect, plugin_info->FLASHPlugin_CheckSectorProtection, loaded_plugin.sp, (uint32_t *)&result, 3, sector, sectors_to_check, plugin_info->WorkArea.Address);
                if (retval != ERROR_OK)
                    break;
